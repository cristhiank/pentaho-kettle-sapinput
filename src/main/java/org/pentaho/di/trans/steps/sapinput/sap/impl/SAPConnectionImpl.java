...
public class SAPConnectionImpl implements SAPConnection {
	...
	public void open(SAPConnectionParams params) throws SAPException {
		...
		try {
			open( connectionname, connectProperties );
		}
	...
	private void open(String connectionName, Properties props) throws JCoException, SAPException {
		try {
			destination = KettleRFCDestination.getDestination( connectionName, props );
		}
		catch (UnsatisfiedLinkError e) {
			e.printStackTrace();
			String message = BaseMessages.getString( SapInputMeta.class,
